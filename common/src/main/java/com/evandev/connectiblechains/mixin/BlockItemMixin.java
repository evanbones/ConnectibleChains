...
public class BlockItemMixin {
    ...
    private boolean connectiblechains$rejectHangingBlockOverlap(boolean original, BlockPlaceContext context, BlockState state) {
        if (!original) return false;

        Player player = context.getPlayer();
        CollisionContext collisionContext = player == null ? CollisionContext.empty() : CollisionContext.of(player);
        return !HangingBlockPlacement.isObstructedByHangingBlock(
                context.getLevel(), state, context.getClickedPos(), collisionContext);
    }
}
